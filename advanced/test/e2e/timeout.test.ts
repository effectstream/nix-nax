...
describe("e2e: timeout / forfeit", () => {
  test("O stops responding -> X startTimeout + claimTimeout -> X wins", async () => {
    ...
    //
    // PRE-REPAIR OBSERVATION: an opening single-move settle was rejected by the
    // then-current local wallet/node stack with FeeCalculation, while simulator,
    // later-state single-move and sampled multi-move submissions succeeded. This
    // retained optional scenario opens with a multi-move chunk; it does not claim
    // every repaired-source multi-move shape has been measured on the current node.
    const P5 = [
    ...
    // The timeout decision preserves the opening settlement's optimistic
    // challenge right. Finalization becomes valid only after that stored cutoff.
    await waitUntilAfter(untilTime);
