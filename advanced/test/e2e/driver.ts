...
export const E2E_MAX_WINDOW = 600n;
...
export interface GameHandle {
  ...
  requestEntropy(as: "x" | "o", turn: number, respondBy: bigint): Promise<string>;
  answerEntropy(turn: number): Promise<string>;
  claimEntropyTimeout(): Promise<string>;
  challengeRoll(turn: number, respondBy: bigint): Promise<string>;
  answerRollChallenge(turn: number): Promise<string>;
  proveEquivocationByX(turn: number, a: ScriptMove, b: ScriptMove): Promise<string>;
  readDyn(): Promise<GameDynView>;
  readEntropyResponse(turn: number): Promise<any | null>;
...
export async function openGame(pair: TestPair): Promise<GameHandle> {
  ...
  return {
    ...
    async requestEntropy(as, turn, respondBy) {
      const mover = as === "x" ? pair.x : pair.o;
      const it = intentFor(pair, turn);
      const { found } = await attachWithSecret({
        contractAddress: a.contractAddress,
        wallet: a.wallet,
        secret: mover.secret,
        storeSuffix: `e2e-er-${as}-${Buffer.from(pair.gameId).toString("hex").slice(0, 8)}`,
      });
      const t: any = await submitWithRetry("requestEntropy", () => (found as any).callTx.requestEntropy(
        pair.gameId,
        BigInt(it.slot),
        ...it.bits.map(BigInt),
        it.secret,
    ...
    async answerEntropy(turn) {
      const it = intentFor(pair, turn);
      const rr = revealFor(pair, turn, it.slot);
      const t: any = await submitWithRetry("answerEntropy", () => (a.found as any).callTx.answerEntropy(
        pair.gameId,
        ...rr.bits.map(BigInt),
        rr.random,
        rr.path,
    ...
    async claimEntropyTimeout() {
      const t: any = await submitWithRetry("claimEntropyTimeout", () =>
        (a.found as any).callTx.claimEntropyTimeout(pair.gameId));
      return t.public.txId as string;
    },
    async challengeRoll(turn, respondBy) {
      const responder = responderOf(pair, turn);
      const { found } = await attachWithSecret({
        contractAddress: a.contractAddress,
        wallet: a.wallet,
        secret: responder.secret,
        storeSuffix: `e2e-rc-${turn}-${Buffer.from(pair.gameId).toString("hex").slice(0, 8)}`,
      });
      const t: any = await submitWithRetry("challengeRoll", () =>
        (found as any).callTx.challengeRoll(pair.gameId, BigInt(turn), respondBy));
      return t.public.txId as string;
    },
    async answerRollChallenge(turn) {
      const it = intentFor(pair, turn);
      const rr = revealFor(pair, turn, it.slot);
      const t: any = await submitWithRetry("answerRollChallenge", () =>
        (a.found as any).callTx.answerRollChallenge(
          pair.gameId,
          BigInt(it.slot),
          ...it.bits.map(BigInt), it.secret, it.path,
          ...rr.bits.map(BigInt), rr.random, rr.path,
        ));
      return t.public.txId as string;
    },
    ...
    async readDyn() {
      ...
      return {
        ...
        hasRollChallenge: d.hasRollChallenge as boolean,
      };
    },
    async readEntropyResponse(turn) {
      const led = await readLedger(a.providers, a.contractAddress);
      const responses = (led as any).entropyResponses.lookup(pair.gameId);
      return responses.member(BigInt(turn)) ? responses.lookup(BigInt(turn)) : null;
    },
