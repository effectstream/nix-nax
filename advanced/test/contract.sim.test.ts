...
import { Contract, ledger, Status, Winner } from "../contract/managed/contract/index.js";
import { createWitnesses, createNixNaxPrivateState } from "../contract/witnesses.ts";
import {
  playersA,
  playersB,
  playersC,
...
function setup(pair: TestPair, playerSecret?: Uint8Array) {
  ...
  const deployed = contract.initialState(ctorCtx, SIM_MIN_WINDOW, SIM_MAX_WINDOW);
...
describe("roll-class dispute", () => {
  ...
  test("unanswered challenge: mover forfeits after respondBy; winner mints via claimResult", () => {
    ...
    // The roll forfeit does not truncate the optimistic history window.
    ctx = newCircuitCtx(st, privateState, 4999);
    expect(() => (contract.impureCircuits as any).claimResult(ctx, pair.gameId, RECIP))
      .toThrow(/challenge window still open/);
    // O (the winner) finalises + mints at the optimistic cutoff.
    ctx = newCircuitCtx(st, privateState, 5000);
    const fin = (contract.impureCircuits as any).claimResult(ctx, pair.gameId, RECIP);
    const d = dynOf(led(fin.context.currentQueryContext.state), pair);
    expect(d.status).toBe(Status.settled);
    expect(d.winner).toBe(Winner.o);
...
// ── Responder data availability (C4) ───────────────────────────────────────
describe("entropy request / public answer / responder forfeit", () => {
  const requestArgs = (pair: TestPair, turn: number, by: bigint) => {
    const it = intentFor(pair, turn);
    return [
      BigInt(it.slot), ...it.bits.map(BigInt), it.secret, it.path, by,
    ] as const;
  };
  const answerArgs = (pair: TestPair, turn: number) => {
    const it = intentFor(pair, turn);
    const rv = revealFor(pair, turn, it.slot);
    return [...rv.bits.map(BigInt), rv.random, rv.path] as const;
  };

  function honestAnswerForTurn(turn: 1 | 2) {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    const opening = turn === 1 ? [P(0, 0)] : [P(0, 0), P(4, 0)];
    let st = settleChunk(contract, state, privateState, pair, 0, opening);
    const mover = turn % 2 === 0 ? "x" : "o";
    const waiter = mover === "x" ? "o" : "x";
    let ctx = newCircuitCtx(st, secretPS(pair, waiter), 1050);
    st = (contract.impureCircuits as any).startTimeout(
      ctx, pair.gameId, 1800n,
    ).context.currentQueryContext.state;
    expect(dynOf(led(st), pair).hasDeadline).toBe(true);
    ctx = newCircuitCtx(st, secretPS(pair, mover), 1100);
    const requested = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, turn, 1800n),
    );
    st = requested.context.currentQueryContext.state;
    expect(led(st).entropyRequests.member(pair.gameId)).toBe(true);
    expect(dynOf(led(st), pair).hasDeadline).toBe(false);

    ctx = newCircuitCtx(st, privateState, 1200);
    const answered = (contract.impureCircuits as any).answerEntropy(
      ctx, pair.gameId, ...answerArgs(pair, turn),
    );
    st = answered.context.currentQueryContext.state;
    const l = led(st);
    expect(l.entropyRequests.member(pair.gameId)).toBe(false);
    expect(l.entropyResponses.lookup(pair.gameId).member(BigInt(turn))).toBe(true);
    expect(l.rollAnswered.lookup(pair.gameId).member(BigInt(turn))).toBe(false);
    const publicResponse = l.entropyResponses.lookup(pair.gameId).lookup(BigInt(turn));
    const expected = revealFor(pair, turn, intentFor(pair, turn).slot);
    expect(Number(publicResponse.slot)).toBe(expected.slot);
    expect([publicResponse.b0, publicResponse.b1, publicResponse.b2, publicResponse.b3].map(Number)).toEqual(expected.bits);
    expect(Buffer.from(publicResponse.random)).toEqual(Buffer.from(expected.random));
    expect(publicResponse.path.path.map((p: any) => p.sibling.field)).toEqual(expected.path.path.map((p) => p.sibling.field));
    const publicReveal: RandomReveal = {
      channelId: hexOf(pair.gameId),
      turn,
      slot: Number(publicResponse.slot),
      bits: [publicResponse.b0, publicResponse.b1, publicResponse.b2, publicResponse.b3].map(Number),
      random: publicResponse.random,
      path: publicResponse.path,
    };
    expect(verifyRandomReveal(
      publicReveal,
      hexOf(pair.gameId),
      turn,
      Number(publicResponse.slot),
      responderOf(pair, turn).random.root.field,
    )).toEqual({ ok: true });

    // Publication is one-shot and normal play can continue using that reveal.
    ctx = newCircuitCtx(st, secretPS(pair, mover), 1250);
    expect(() => (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, turn, 1900n),
    )).toThrow(/already published/);
    ctx = newCircuitCtx(st, secretPS(pair, waiter), 1250);
    st = (contract.impureCircuits as any).startTimeout(
      ctx, pair.gameId, 2000n,
    ).context.currentQueryContext.state;
    expect(dynOf(led(st), pair).hasDeadline).toBe(true);
    const move = turn === 1 ? P(4, 0) : P(1, 0);
    st = settleChunk(contract, st, secretPS(pair, mover), pair, turn, [move], { time: 1300, until: 3000n });
    expect(Number(dynOf(led(st), pair).committedTurns)).toBe(turn + 1);
    expect(dynOf(led(st), pair).hasDeadline).toBe(false);
  }

  test("both player roles can request, publish reusable R proof, and continue", () => {
    honestAnswerForTurn(1);
    honestAnswerForTurn(2);
  });

  test("answer is strictly before the cutoff; forfeit at cutoff awards the mover", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    let st = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]); // O moves at t1
    let ctx = newCircuitCtx(st, secretPS(pair, "o"), 1100);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 1, 1800n),
    ).context.currentQueryContext.state;
    ctx = newCircuitCtx(st, privateState, 1799);
    expect(() => (contract.impureCircuits as any).claimEntropyTimeout(ctx, pair.gameId))
      .toThrow(/response window still open/);
    ctx = newCircuitCtx(st, privateState, 1800);
    expect(() => (contract.impureCircuits as any).answerEntropy(ctx, pair.gameId, ...answerArgs(pair, 1)))
      .toThrow(/response window has closed/);
    const won = (contract.impureCircuits as any).claimEntropyTimeout(ctx, pair.gameId);
    const finalState = won.context.currentQueryContext.state;
    expect(dynOf(led(finalState), pair).winner).toBe(Winner.o);
    expect(led(finalState).entropyRequests.member(pair.gameId)).toBe(false);
  });

  test("wrong caller and wrong-slot R proof reject atomically", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    let st = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]); // O moves at t1
    let ctx = newCircuitCtx(st, secretPS(pair, "x"), 1100);
    expect(() => (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 1, 1800n),
    )).toThrow(/only the current mover/);

    ctx = newCircuitCtx(st, secretPS(pair, "o"), 1100);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 1, 1800n),
    ).context.currentQueryContext.state;
    const requested = st;
    const it = intentFor(pair, 1);
    const wrong = revealFor(pair, 1, (it.slot + 1) % 16);
    ctx = newCircuitCtx(st, privateState, 1200);
    expect(() => (contract.impureCircuits as any).answerEntropy(
      ctx, pair.gameId, ...wrong.bits.map(BigInt), wrong.random, wrong.path,
    )).toThrow(/canonical R-leaf/);
    expect(led(requested).entropyRequests.member(pair.gameId)).toBe(true);
    expect(led(requested).entropyResponses.lookup(pair.gameId).member(1n)).toBe(false);
  });

  test("a noncanonical R path under a malicious registered root is rejected", () => {
    const base = playersA();
    const it = intentFor(base, 1);
    const mal = buildMaliciousRandomTree(base.gameId, 1, it.slot, 999);
    const pair: TestPair = { ...base, x: { ...base.x, random: { ...base.x.random, root: mal.root } } };
    const { contract, privateState, state } = setup(pair);
    let st = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]);
    let ctx = newCircuitCtx(st, secretPS(pair, "o"), 1100);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 1, 1800n),
    ).context.currentQueryContext.state;
    ctx = newCircuitCtx(st, privateState, 1200);
    expect(() => (contract.impureCircuits as any).answerEntropy(
      ctx, pair.gameId, ...mal.bits.map(BigInt), mal.random, mal.path,
    )).toThrow(/canonical R-leaf/);
  });

  test("an expired move timer cannot be replaced by an entropy request", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    let st = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]);
    let ctx = newCircuitCtx(st, secretPS(pair, "x"), 1100);
    st = (contract.impureCircuits as any).startTimeout(ctx, pair.gameId, 1800n).context.currentQueryContext.state;
    ctx = newCircuitCtx(st, secretPS(pair, "o"), 1800);
    expect(() => (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 1, 2500n),
    )).toThrow(/move deadline has already expired/);
  });

  test("earlier roll dispute cancels an unpublished request, then permits re-request", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    let st = settleChunk(contract, state, privateState, pair, 0, [P(0, 0), P(4, 0), P(1, 0)]); // O t3
    let ctx = newCircuitCtx(st, secretPS(pair, "o"), 1100);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 3, 1800n),
    ).context.currentQueryContext.state;
    expect(led(st).entropyRequests.member(pair.gameId)).toBe(true);
    // X is responder of O's earlier turn 1.
    ctx = newCircuitCtx(st, secretPS(pair, "x"), 1150);
    st = (contract.impureCircuits as any).challengeRoll(ctx, pair.gameId, 1n, 1900n).context.currentQueryContext.state;
    expect(led(st).entropyRequests.member(pair.gameId)).toBe(false);
    const it1 = intentFor(pair, 1);
    const rv1 = revealFor(pair, 1, it1.slot);
    ctx = newCircuitCtx(st, privateState, 1200);
    st = (contract.impureCircuits as any).answerRollChallenge(
      ctx, pair.gameId, BigInt(it1.slot), ...it1.bits.map(BigInt), it1.secret, it1.path,
      ...rv1.bits.map(BigInt), rv1.random, rv1.path,
    ).context.currentQueryContext.state;
    ctx = newCircuitCtx(st, secretPS(pair, "o"), 1250);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 3, 2000n),
    ).context.currentQueryContext.state;
    expect(led(st).entropyRequests.lookup(pair.gameId).turn).toBe(3n);
  });

  test("published entropy does not suppress a false-class roll challenge", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    const opening = [P(2, 0), P(4, 0), P(0, 0), P(15, 0), P(1, 0)];
    let st = settleChunk(contract, state, privateState, pair, 0, opening); // O t5, true remove class
    let ctx = newCircuitCtx(st, secretPS(pair, "o"), 1100);
    st = (contract.impureCircuits as any).requestEntropy(
      ctx, pair.gameId, ...requestArgs(pair, 5, 1800n),
    ).context.currentQueryContext.state;
    ctx = newCircuitCtx(st, privateState, 1200);
    st = (contract.impureCircuits as any).answerEntropy(
      ctx, pair.gameId, ...answerArgs(pair, 5),
    ).context.currentQueryContext.state;
    expect(led(st).rollAnswered.lookup(pair.gameId).member(5n)).toBe(false);
    st = settleChunk(contract, st, secretPS(pair, "o"), pair, 5,
      [{ kind: KIND_PLACE, cell: 9, size: 0, lieParity: 1 }], { time: 1300, until: 3000n });
    ctx = newCircuitCtx(st, secretPS(pair, "x"), 1400);
    st = (contract.impureCircuits as any).challengeRoll(ctx, pair.gameId, 5n, 2101n).context.currentQueryContext.state;
    expect(dynOf(led(st), pair).hasRollChallenge).toBe(true);
    const it = intentFor(pair, 5);
    const rv = revealFor(pair, 5, it.slot);
    ctx = newCircuitCtx(st, privateState, 1500);
    expect(() => (contract.impureCircuits as any).answerRollChallenge(
      ctx, pair.gameId, BigInt(it.slot), ...it.bits.map(BigInt), it.secret, it.path,
      ...rv.bits.map(BigInt), rv.random, rv.path,
    )).toThrow(/reveals contradict the claimed class/);
  });
});
...
describe("security regressions", () => {
  ...
  test("C2: settle rejects the original multi-decade challenge deadline", () => {
    const pair = playersC();
    const { contract, privateState, state } = setup(pair);
    expect(() =>
      settleChunk(contract, state, privateState, pair, 0, WIN_X, {
        time: 1000,
        until: 4_000_000_000n,
      }),
    ).toThrow(/deadline too far/);
  });
...
describe("cheat: winner-flip during the challenge window", () => {
  ...
  test("a roll challenge cancels an armed timeout and resolution permits fresh grace", () => {
    const pair = playersA();
    const { contract, privateState, state } = setup(pair);
    ...
    expect(dynOf(led(st), pair).hasRollChallenge).toBe(true);
    expect(dynOf(led(st), pair).hasDeadline).toBe(false);
    // The canceled timeout cannot decide the game during the roll dispute.
    ...
    const rv = revealFor(pair, 1, it.slot);
    ctx = newCircuitCtx(st, privateState, 1500);
    const answered = (contract.impureCircuits as any).answerRollChallenge(
      ctx, pair.gameId, BigInt(it.slot),
      ...it.bits.map(BigInt), it.secret, it.path,
      ...rv.bits.map(BigInt), rv.random, rv.path,
    );
    st = answered.context.currentQueryContext.state;
    expect(dynOf(led(st), pair).hasRollChallenge).toBe(false);
    // A newly armed timer receives a full grace after dispute resolution.
    ctx = newCircuitCtx(st, secretPS(pair, waiter), 1600);
    const fresh = (contract.impureCircuits as any).startTimeout(ctx, pair.gameId, 2301n);
    expect(Number(dynOf(led(fresh.context.currentQueryContext.state), pair).deadline)).toBe(2301);
  });
});
...
describe("cheat: adversarial constructions", () => {
  test("a stashed alternate I leaf is rejected at its noncanonical path position", () => {
    ...
    const ctx = newCircuitCtx(state, privateState, 1000);
    expect(() => (contract.impureCircuits as any).proveIndexEquivocationByO(
    ...
    expect(dynOf(led(state), pair).winner).toBe(Winner.none);
  });
  ...
  test("a lying division witness only fails the cheater's own settle", () => {
    ...
    const ctorCtx = createConstructorContext(privateState, ZERO_KEY as any);
    let state = contract.initialState(ctorCtx, SIM_MIN_WINDOW, SIM_MAX_WINDOW).currentContractState;
...
describe("boundaries: windows, chunk size, ranges", () => {
  test("constructor rejects zero, reversed/equal, and over-seven-day windows", () => {
    const pair = playersC();
    const privateState = createNixNaxPrivateState(pair.x.secret);
    const deploy = (min: bigint, max: bigint) => {
      const contract = new Contract(createWitnesses() as any);
      return contract.initialState(createConstructorContext(privateState, ZERO_KEY as any), min, max);
    };
    expect(() => deploy(0n, 10n)).toThrow(/minimum window must be nonzero/);
    expect(() => deploy(10n, 10n)).toThrow(/maximum window must exceed minimum/);
    expect(() => deploy(11n, 10n)).toThrow(/maximum window must exceed minimum/);
    expect(() => deploy(600n, 604801n)).toThrow(/seven-day protocol bound/);
    expect(() => deploy(600n, 604800n)).not.toThrow();
  });
  ...
  test("deadline upper bound accepts exact max and rejects max+1 and Uint64 max", () => {
    const pair = playersC();
    {
      const { contract, privateState, state } = setup(pair);
      const s1 = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]);
      const ctx = newCircuitCtx(s1, privateState, 1000);
      const armed = (contract.impureCircuits as any).startTimeout(ctx, pair.gameId, 11000n);
      expect(Number(dynOf(led(armed.context.currentQueryContext.state), pair).deadline)).toBe(11000);
    }
    for (const tooFar of [11001n, (1n << 64n) - 1n]) {
      const p = playersD();
      const { contract, privateState, state } = setup(p);
      const s1 = settleChunk(contract, state, privateState, p, 0, [P(0, 0)]);
      const ctx = newCircuitCtx(s1, privateState, 1000);
      expect(() => (contract.impureCircuits as any).startTimeout(ctx, p.gameId, tooFar))
        .toThrow(/deadline too far/);
    }
  });

  test("new fraud and roll challenges reject at the optimistic cutoff", () => {
    const pair = playersC();
    const { contract, privateState, state } = setup(pair);
    const s1 = settleChunk(contract, state, privateState, pair, 0, WIN_X, { time: 1000, until: 2000n });
    let ctx = newCircuitCtx(s1, secretPS(pair, "o"), 2000);
    expect(() => (contract.impureCircuits as any).challengeRoll(ctx, pair.gameId, 2n, 2701n))
      .toThrow(/outcome no longer contestable/);
    ctx = newCircuitCtx(s1, secretPS(pair, "o"), 2000);
    expect(() => (contract.impureCircuits as any).proveEquivocationByX(
      ctx, pair.gameId, 2n,
      1n, 1n, 0n, pair.x.token.secrets[2][1], pair.x.token.pathFor(2, KIND_PLACE, 1, 0),
      1n, 2n, 0n, pair.x.token.secrets[2][2], pair.x.token.pathFor(2, KIND_PLACE, 2, 0),
    )).toThrow(/outcome no longer contestable/);
  });
  ...
  test("startTimeout on a half-open (not-yet-joined) game is rejected", () => {
    ...
    const privateState = createNixNaxPrivateState(pair.x.secret);
    const contract = new Contract(createWitnesses() as any);
    let state = contract.initialState(createConstructorContext(privateState, ZERO_KEY as any), SIM_MIN_WINDOW, SIM_MAX_WINDOW).currentContractState;
  ...
  test("startTimeout cannot be re-armed or extended while active", () => {
    const pair = playersC();
    const { contract, privateState, state } = setup(pair);
    const s1 = settleChunk(contract, state, privateState, pair, 0, [P(0, 0)]);
    let ctx = newCircuitCtx(s1, secretPS(pair, "x"), 1000);
    ...
    // Re-arm with a later deadline is rejected; the first obligation stays.
    ctx = newCircuitCtx(st, secretPS(pair, "x"), 1100);
    expect(() => (contract.impureCircuits as any).startTimeout(ctx, pair.gameId, 3000n))
      .toThrow(/timeout already armed/);
    expect(Number(dynOf(led(st), pair).deadline)).toBe(2000);
  });
});
...
// entry points; retained historical variants are checked against current
// generated metadata and live-node evidence rather than assumed circuit size.
