...
// `persistentHash("nixnax:id:", gameId, secret) == idX || idO`, so the caller cannot
...
export type NixNaxPrivateState = {
  // The player's 32-byte identity secret. Player-authenticated circuits use it
  // for callerMark; permissionless proof/claim circuits ignore its value.
  secret: Uint8Array;
};
