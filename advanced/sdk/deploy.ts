...
import { buildProviders } from "../../src/sdk/providers.ts";
...
export const DEPLOYMENT_FILE = process.env.MIDNIGHT_DEPLOYMENT_FILE
  ...
  : path.resolve(fileURLToPath(new URL("..", import.meta.url)), "nixnax.advanced.undeployed.json");

const DEFAULT_PRIVATE_STATE_ID = "nixnaxAdvancedArena";
...
export const DEFAULT_MAX_WINDOW_SECS = 3600n;
...
export async function ensureArenaDeployed(opts: {
  ...
  const minWindowSecs = opts.minWindowSecs ?? DEFAULT_MIN_WINDOW_SECS;
  const maxWindowSecs = opts.maxWindowSecs ?? DEFAULT_MAX_WINDOW_SECS;
  ...
  // Reuse only a complete deployment with this revision's exact constructor
  // configuration. findDeployedContract below also compares every maintained
  // verifier key to its on-chain operation and fails closed on stale keys.
  ...
  if (existsSync(deploymentFile)) {
    try {
      ...
      if (saved.contractAddress) {
        ...
        const configured = st ? ledger((st as any).data ?? st) : null;
        if (
          st &&
          circuitIds.every((id) => st.operation(id) !== undefined) &&
          BigInt((configured as any).minWindowSecs) === minWindowSecs &&
          BigInt((configured as any).maxWindowSecs) === maxWindowSecs
        ) {
  ...
  if (!contractAddress) {
    ...
    mkdirSync(path.dirname(deploymentFile), { recursive: true });
