...
// React-free connected-wallet state and selection service. The module-level
// store lets UI adapters and transaction services share one connection.
...
import { invalidateArenaAttachmentSoon } from "../chain/attachment-cache.ts";
...
export const subscribeWallet = (cb: () => void): (() => void) => {
  store.subs.add(cb);
...
export async function connect(wallet: InitialAPI): Promise<void> {
  ...
  try {
    ...
    // next on-chain action attaches via this connection.
    invalidateArenaAttachmentSoon();
